...
ObjectPool<SpriteSheet>* SpriteSheet::Pool = nullptr;
...
void SpriteSheet::SetSize(byte _rows, byte _columns, byte _sizeX, byte _sizeY) {
	m_rows = _rows;
	m_columns = _columns;
	m_clipSizeX = _sizeX;
	m_clipSizeY = _sizeY;
...
Rect SpriteSheet::Update(AnimationNames _name, float _deltaTime) {
	...
	if (_name == EN_AN_DEATH) {
		m_animations[_name]->UpdateFinal(_deltaTime);
	}
	else {
